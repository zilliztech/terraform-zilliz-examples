...
resource "google_service_account_iam_member" "cluster-workload-identity" {
  ...
  member  = join("", [
    ...
    "/projects/${data.google_project.project.number}", // project number
    "/locations/global",
    "/workloadIdentityPools/${var.gcp_project_id}.svc.id.goog", // default identity pool
    "/kubernetes.cluster/",
    "https://container.googleapis.com/v1",
    "/projects/${var.gcp_project_id}",
    "/locations/${var.gcp_region}",
    "/clusters/${var.gke_cluster_name}" // gke cluster
  ])
}